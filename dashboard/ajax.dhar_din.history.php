...
$sql = "SELECT * FROM dhar_din_copy WHERE gid='$gid' AND user='' AND phone=''";
$m  = mysqli_query($db,$sql);
...
<?php 
$sql = "SELECT * FROM dhar_din_copy WHERE new=1 AND user='$user' AND phone='$phone'";
$m  = mysqli_query($db,$sql);
if (mysqli_num_rows($m)==0) {
	echo "No Collection Detail Found";
} else {
?>
<table class="input_table res1">
	<tr>
