...
<ul>
  ...
  <li><a class="active" href="dhar_din.php">ধার দিন</a></li>
...
if (isset($_POST['submit'])) {
  ...
  $gid= time();
  $sql = "INSERT INTO `dhar_din` (`id`, `user`, `phone`, `address`, `gid`, `amount`,`paid`, `des`, `date`, `adder`) VALUES (NULL, '$name', '$phone', '$address', '$gid', '$total','$paid', '$description', '$date', '$adder')";
  mysqli_query($db,$sql);
   $sql = "INSERT INTO `dhar_din_copy` (`id`, `user`, `phone`, `address`, `gid`, `amount`,`paid`, `des`, `date`, `adder`, `pre`, `new`) VALUES (NULL, '$name', '$phone', '$address', '$gid', '$total','$paid', '$description', '$date', '$adder', '0', '0')";
  if (mysqli_query($db,$sql)) {
    ?>
<div class="ani_top success">সংযোজন হয়েছে। <a href="print_dhar_din.php?gid=<?php echo $gid ?>" target="_blank">প্রিন্ট করুন</a></div>
    <?php
  }
}
...
buttons:{
  "close":function(){
    ...
    {
      $.ajax({
        url: 'ajax.dhar_din.user_change.php',
        type: 'POST',
        data: "id="+id,
      })
      .done(function(data) {
    ...
    {
      $.ajax({
        url: 'ajax.dhar_din.amount_change.php',
        type: 'POST',
        data: "id="+id,
      })
      .done(function(data) {
