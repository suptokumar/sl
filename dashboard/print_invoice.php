...
$gid = $_GET['gid'];
if (!isset($_GET['id'])) {
$sql = "SELECT * FROM drive_copy WHERE g_id='$gid' LIMIT 1";
	
} else {
$id = $_GET['id'];
$sql = "SELECT * FROM drive_copy WHERE id='$id'";
}
$m = mysqli_query($db,$sql);
if (mysqli_num_rows($m)==0) {
	echo "Sorry There is an Error. Target File wasn't Found. <a href='index_report.php'>Go to Report File</a> ";
}
	...
	<div class="container">
		...
		<table style="border-collapse: collapse; margin: 0px auto; width: 96%;">
			<tr>
				...
				<td>
					Serial: <?php echo $row['serial'] ?>
				</td>
			</tr>
			<tr>
				<td>
					Refference Name: <?php echo $row['r_name'] ?>
				</td>
				<td>
					Refference Phone Number: <?php echo $row['r_phone'] ?>
				</td>
				<td>
		...
		<table style="border-collapse: collapse; margin: 0px auto; width: 96%;">
			...
			<tr>
				...
				<td>
					<?php echo date("h:ia ,d M Y",strtotime($row['DCTB'])) ?>
				</td>
			</tr>
			<tr>
				<td>
				...
				<td>
					<?php echo date("h:ia ,d M Y",strtotime($row['DCTB_date'])) ?>
				</td>
			</tr>
			<tr>
				<td>
