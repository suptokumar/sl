...
<body>
	...
	<div class="container">
		...
		<?php 
$gid = $row['gid'];
$sql= "SELECT SUM(paid) FROM dhar_nin_copy WHERE gid='$gid' ORDER BY id DESC";
$g = mysqli_query($db,$sql);
$f = mysqli_fetch_array($g);
$sql= "SELECT paid FROM dhar_nin_copy WHERE gid='$gid' ORDER BY id DESC LIMIT 1";
$g = mysqli_query($db,$sql);
$st = mysqli_fetch_array($g);
		...
		<a href="dhar_nin.php" style="text-decoration: none; color: #11A3FF; font-family: arial;">◄ Back</a>
		</div>
	</div>
</body>
</html>
...
if (isset($_GET['gid'])) {
	$id = $_GET['gid'];
$sql = "SELECT * FROM dhar_nin_copy WHERE gid='$id'";
	// $blog = "dhar_din";
...
if (isset($_GET['cid'])) {
	$id = $_GET['cid'];
$sql = "SELECT * FROM dhar_nin_copy WHERE id='$id'";
	// $blog = "dhar_din";
