...
<?php 
$sql = "SELECT SUM(amount),SUM(paid) FROM dhar_din WHERE user LIKE '%$search%' OR phone LIKE '%$search%'";
...
$sql = "SELECT * FROM dhar_din WHERE user LIKE '%$search%' OR phone LIKE '%$search%' GROUP BY user,phone ORDER BY CASE
...
<tr id="d<?php echo $row['id'] ?>">
 ...
 <td><?php echo $row['phone'] ?></td>
  ...
  <td><?php echo total_amount($row['user'],$row['phone']) ?> Taka</td>
  <td><?php echo total_paid($row['user'],$row['phone']) ?> Taka</td>
  <td><?php echo (total_amount($row['user'],$row['phone']))-(total_paid($row['user'],$row['phone'])) ?> Taka</td>
  ...
  <td><a href="javascript:void(0)" onclick="open_detail('<?php echo $row['id'] ?>')" class='button green'>Detail</a>
  	<?php if ((total_amount($row['user'],$row['phone']))-(total_paid($row['user'],$row['phone']))>0) {
  		?>
