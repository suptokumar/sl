...
if ($search!='') {
  $ql .=" AND (name LIKE '%$search%' OR phone LIKE '%$search%' OR serial='$search') ";
}
$sql = "SELECT * FROM drive WHERE hide=0 ".$ql." ORDER BY CASE
WHEN serial='$search' THEN 0
...
?>
<tr id="p<?php echo $row['id'] ?>">
  <td style="width: 4%;"><?php echo $row['serial'] ?></td>
  <td><?php echo $row['name'] ?></td>
  <td class="auto_hide"><?php echo $row['phone'] ?></td>
